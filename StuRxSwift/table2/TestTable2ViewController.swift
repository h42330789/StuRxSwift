...
class TestTable2ViewController: UIViewController {
    ...
    lazy var dataSource = {
        let dt = RxTableViewSectionedReloadDataSource<SectionModel<String, String>> { _, tableView, indexPath, element in
...
class TestMyTable2ViewController: UIViewController {
    ...
    lazy var dataSource = {
        let dt = RxTableViewSectionedReloadDataSource<MySection> { _, tableView, indexPath, element in
            let cell = tableView.dequeueReusableCell(withIdentifier: "Cell")!
            cell.textLabel?.text = "\(indexPath.row)：\(element)"
            return cell
        }
